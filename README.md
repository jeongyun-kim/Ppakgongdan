...
**✓ Tuist를 활용한 모듈화**
<br>
<img src="https://github.com/user-attachments/assets/b9a3052a-a4ca-46d2-8e3b-dd0c64ebd18f" width="100%" height="30%">
<br>

```Tuist를 이용하여 각 기능별 관심사를 분리```하는 모듈화 작업을 수행하였습니다. <br>

- UI
    - 앱에서 공통으로 사용되는 CustomView 또는 컬러 색상 등은 자주 생성되고 수정이 될 수 있기 때문에 다른 모듈과의 연관을 두지 않고 완전 독립적으로 존재하여 수정되더라도 타 모듈에 영향이 가지 않게 구성하였습니다.
- Feature
    - 모든 기능을 아우르는 모듈로 새로운 기능 개발 구현 시, ‘Feature_기능’으로 하여 새로운 기능이 추가되는 방향으로 구성하였습니다. 이때에는 대규모 개발이 아니기 때문에 View와 핵심 Feature가 거의 동일하다고 판단되어 각 View를 기준으로 진행하였습니다. 이때, 각 기능이 다른 기능을 의존하지 않아 언제든 새로운 기능을 추가하거나 수정 및 삭제한다 해도 다른 개별적인 모듈에 영향을 미치지 않아 새롭게 코드를 변경할 번거로움이 존재하지 않습니다.
- Database
    - 로컬 데이터베이스의 경우 네트워크와 달리 모든 화면에서 사용되지 않고 특정 화면에서만 사용되며, Repository Pattern을 이용해 언제나 하나의 인스턴스만으로 데이터에 접근할 수 있게 설계해 Feature로 직접적인 주입이 없이 중앙에서 관리할 수 있도록 하였습니다.
- NetworkKit
    - NetworkKit의 경우, Database 모듈과 달리 모든 화면에서 사용되는 핵심으로 생각하여 의존성을 직접 주입해 주는 방향으로 결정하였습니다. 이러한 과정에서 각 화면 별 통신에 관한 프로토콜을 정의하여 이를 하나의 클래스에 extension 해주는 방식으로 처리하였습니다. 때문에 NetworkKit에 새로운 코드가 추가되거나 변경된다고 하더라도 다른 모듈에는 영향을 미치지 않아 유지보수성과 코드 재사용성이 지켜질 수 있었습니다.
- Utils
    - 모든 라이브러리가 설치된 모듈로, 개발 초기에 어떤 기능을 구상하고 해당 기능을 구현하기 위한 라이브러리를 초기에 결정된다 생각하여 많은 변경 요소가 적을 것 같아 최하위 모듈로 분리하였습니다.
